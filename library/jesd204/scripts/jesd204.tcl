...
proc adi_axi_jesd204_tx_create {ip_name num_lanes {num_links 1} {mode64b 0}} {
  ...
  set result [catch {
    ...
    ad_ip_parameter "${ip_name}/tx_axi" CONFIG.MODE_64B66B_8B10B_N $mode64b
    ad_ip_parameter "${ip_name}/tx"     CONFIG.NUM_LANES $num_lanes
    ad_ip_parameter "${ip_name}/tx"     CONFIG.NUM_LINKS $num_links
    ad_ip_parameter "${ip_name}/tx"     CONFIG.MODE_64B66B_8B10B_N $mode64b

    ad_connect "${ip_name}/tx_axi/core_reset" "${ip_name}/tx/reset"
    if {$mode64b == 0} {ad_connect "${ip_name}/tx_axi/tx_ctrl" "${ip_name}/tx/tx_ctrl"}
    ad_connect "${ip_name}/tx_axi/tx_cfg" "${ip_name}/tx/tx_cfg"
    ad_connect "${ip_name}/tx/tx_event" "${ip_name}/tx_axi/tx_event"
    if {$mode64b == 0} {ad_connect "${ip_name}/tx/tx_status" "${ip_name}/tx_axi/tx_status"}
    if {$mode64b == 0} {ad_connect "${ip_name}/tx/tx_ilas_config" "${ip_name}/tx_axi/tx_ilas_config"}
    ...
    if {$mode64b == 0} {create_bd_pin -dir I -from [expr $num_links - 1] -to 0 "${ip_name}/sync"}
    ...
    if {$mode64b == 0} {ad_connect "${ip_name}/sync" "${ip_name}/tx/sync"}
